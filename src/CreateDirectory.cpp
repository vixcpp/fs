...
#include <vix/fs/CreateDirectory.hpp>
#include <vix/fs/FsError.hpp>

namespace vix::fs
{

  FsBoolResult create_directory(std::string_view path)
  {
    if (path.empty())
    {
    ...
    try
    {
      ...
      return std::filesystem::create_directory(fs_path);
    }
    ...
    catch (const std::exception &e)
    {
      return make_fs_error(
          FsErrorCode::Unknown,
          std::string("unexpected error while creating directory: ") + e.what());
    }
    catch (...)
    {
      return make_fs_error(
